...
class QRiskNetwork(nn.Module):
    ...
    def __init__(self, state_size, action_size, risk_size, seed, fc1_units=64, fc2_units=64):
        """Initialize parameters and build model.
        Params
        ======
            state_size (int): Dimension of each state
    ...
    def forward(self, state, risk):
        """Build a network that maps state -> action values."""
        x = F.relu(self.fc1(state))
        risk_enc = F.relu(self.risk_fc(risk))
        x = torch.cat([x, risk_enc], 1)
        x = F.relu(self.fc2(x))
        return self.fc3(x)
